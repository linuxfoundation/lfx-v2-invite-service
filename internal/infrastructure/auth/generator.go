...
// invite_uid, return_url, resource_uid, resource_type, and role.
...
func (g *LinkGenerator) Generate(recipientEmail, returnURL, resourceUID, resourceType, role string, expirationDays int) (link, inviteUID string, expiresAt time.Time, err error) {
	now := time.Now()
